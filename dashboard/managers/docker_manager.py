...
class DockerManager:
    ...
    def get_summary(self) -> dict[str, int]:
        ...
        return {
            ...
            "stopped": sum(1 for c in containers if c.get("status") in ("exited", "created")),
            "healthy": sum(1 for c in containers if c.get("health_normalized") == "healthy"),
            "unhealthy": sum(1 for c in containers if c.get("health_normalized") == "unhealthy"),
            "not_monitored": sum(1 for c in containers if c.get("health_normalized") == "not_monitored"),
        }
