...
class BackupManager:
    def __init__(
        self,
        backup_dir: str | Path = "/opt/acserver/backups",
        backup_script: str | Path = "/opt/acserver/scripts/backup.sh",
    ):
    ...
    def create_runtime_backup(self) -> ActionResult:
        ...
        if proc.returncode == 0:
            summary = self._summarize_success(output, backup_name)
            return ActionResult(True, summary, output, backup_name)

        self._mark_failed_backup(backup_name, proc.returncode, output, started)
        summary = self._summarize_failure(proc.returncode, output, backup_name)
        return ActionResult(False, summary, output, backup_name)
    ...
    def _mark_failed_backup(self, backup_name: str | None, exit_code: int, output: str, started: datetime) -> None:
        if not backup_name:
            return
        candidates = list(self.backup_dir.rglob(backup_name))
        folder = next((p for p in candidates if p.is_dir()), None)
        if not folder:
            return
        marker = folder / "dashboard_failure.txt"
        marker.write_text(
            f"Runtime backup failed.\n"
